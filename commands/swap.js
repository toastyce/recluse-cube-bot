exports.run = (client, message, args) => {
    ...
    let fromMemRoles = [];
    let toMemRoles = [];
    let fromMemNick = "Character A";
    let toMemNick = "Character B";
...
fromMemRoles.push(`<@&${message.guild.member(fromMem)._roles.join('> <@&')}>`)
toMemRoles.push(`<@&${message.guild.member(toMem)._roles.join('> <@&')}>`)
client.log.debug(toMemRoles);
     const embed = new client.Discord.MessageEmbed()
         .setAuthor(`<${client.user.username}>`, client.user.avatarURL)
         .setTitle("Swappy")
         .addField("Roles:", `<@&${message.guild.member(toMem)._roles.join('> <@&')}>`, true)
